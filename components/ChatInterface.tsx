...
export default function ChatInterface() {
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex flex-col max-w-5xl mx-auto px-4 pb-0">
      {/* Header */}
      <div className="py-5 flex items-center justify-between border-b border-border">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Haceme cualquier pregunta sobre tus facturas y gastos</p>
        </div>
        <button onClick={clearHistory} className="p-2 text-muted-foreground hover:text-foreground hover:bg-muted rounded-lg transition-colors" title="Borrar historial">
              ...
              {msg.sql && (
                <div className="w-full">
                  ...
                  >
                    ...
                    {showSql === msg.id ? 'Ocultar query SQL' : `Ver query SQL ${msg.rowCount !== undefined ? `(${msg.rowCount} filas)` : ''}`}
                  </button>
                  {showSql === msg.id && (
                    <div className="mt-2 rounded-lg bg-muted p-3 font-mono text-xs text-muted-foreground overflow-x-auto animate-in fade-in">
                      {msg.sql}
                    </div>
                  )}
                  {showSql === msg.id && msg.rawData && msg.rawData.length > 0 && (
                    <div className="mt-2 rounded-lg border border-border overflow-auto max-h-48 animate-in fade-in">
          ...
          <div className="grid grid-cols-2 gap-2">
            {SUGGESTED_QUESTIONS.map(q => (
              <button
                ...
                className="text-left p-3 text-sm border border-border rounded-xl hover:bg-accent hover:border-primary/30 transition-all text-muted-foreground hover:text-foreground"
              >
                {q}
              </button>
            ))}
      ...
      <div className="pb-6 pt-2 border-t border-border">
        <div className="relative flex items-end gap-3 bg-card border border-border rounded-2xl p-3 focus-within:border-primary/50 focus-within:ring-2 focus-within:ring-primary/10 transition-all shadow-sm">
          <textarea
            ...
            placeholder="¿Cuánto gasté en tecnología este mes?..."
